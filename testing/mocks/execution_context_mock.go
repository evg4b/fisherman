...
//go:generate minimock -i fisherman/internal.ExecutionContext -o ./testing/mocks/execution_context_mock.go -n ExecutionContextMock
...
type ExecutionContextMock struct {
	...
	funcCwd          func() (s1 string)
	inspectFuncCwd   func()
	afterCwdCounter  uint64
	beforeCwdCounter uint64
	CwdMock          mExecutionContextMockCwd
...
func NewExecutionContextMock(t minimock.Tester) *ExecutionContextMock {
	...
	m.CwdMock = mExecutionContextMockCwd{mock: m}
...
type mExecutionContextMockCwd struct {
	mock               *ExecutionContextMock
	defaultExpectation *ExecutionContextMockCwdExpectation
	expectations       []*ExecutionContextMockCwdExpectation
}

// ExecutionContextMockCwdExpectation specifies expectation struct of the ExecutionContext.Cwd
type ExecutionContextMockCwdExpectation struct {
	mock *ExecutionContextMock

	results *ExecutionContextMockCwdResults
	Counter uint64
}

// ExecutionContextMockCwdResults contains results of the ExecutionContext.Cwd
type ExecutionContextMockCwdResults struct {
	s1 string
}

// Expect sets up expected params for ExecutionContext.Cwd
func (mmCwd *mExecutionContextMockCwd) Expect() *mExecutionContextMockCwd {
	if mmCwd.mock.funcCwd != nil {
		mmCwd.mock.t.Fatalf("ExecutionContextMock.Cwd mock is already set by Set")
	}

	if mmCwd.defaultExpectation == nil {
		mmCwd.defaultExpectation = &ExecutionContextMockCwdExpectation{}
	}

	return mmCwd
}

// Inspect accepts an inspector function that has same arguments as the ExecutionContext.Cwd
func (mmCwd *mExecutionContextMockCwd) Inspect(f func()) *mExecutionContextMockCwd {
	if mmCwd.mock.inspectFuncCwd != nil {
		mmCwd.mock.t.Fatalf("Inspect function is already set for ExecutionContextMock.Cwd")
	}

	mmCwd.mock.inspectFuncCwd = f

	return mmCwd
}

// Return sets up results that will be returned by ExecutionContext.Cwd
func (mmCwd *mExecutionContextMockCwd) Return(s1 string) *ExecutionContextMock {
	if mmCwd.mock.funcCwd != nil {
		mmCwd.mock.t.Fatalf("ExecutionContextMock.Cwd mock is already set by Set")
	}

	if mmCwd.defaultExpectation == nil {
		mmCwd.defaultExpectation = &ExecutionContextMockCwdExpectation{mock: mmCwd.mock}
	}
	mmCwd.defaultExpectation.results = &ExecutionContextMockCwdResults{s1}
	return mmCwd.mock
}

//Set uses given function f to mock the ExecutionContext.Cwd method
func (mmCwd *mExecutionContextMockCwd) Set(f func() (s1 string)) *ExecutionContextMock {
	if mmCwd.defaultExpectation != nil {
		mmCwd.mock.t.Fatalf("Default expectation is already set for the ExecutionContext.Cwd method")
	}

	if len(mmCwd.expectations) > 0 {
		mmCwd.mock.t.Fatalf("Some expectations are already set for the ExecutionContext.Cwd method")
	}

	mmCwd.mock.funcCwd = f
	return mmCwd.mock
}

// Cwd implements internal.ExecutionContext
func (mmCwd *ExecutionContextMock) Cwd() (s1 string) {
	mm_atomic.AddUint64(&mmCwd.beforeCwdCounter, 1)
	defer mm_atomic.AddUint64(&mmCwd.afterCwdCounter, 1)

	if mmCwd.inspectFuncCwd != nil {
		mmCwd.inspectFuncCwd()
	}

	if mmCwd.CwdMock.defaultExpectation != nil {
		mm_atomic.AddUint64(&mmCwd.CwdMock.defaultExpectation.Counter, 1)

		mm_results := mmCwd.CwdMock.defaultExpectation.results
		if mm_results == nil {
			mmCwd.t.Fatal("No results are set for the ExecutionContextMock.Cwd")
		}
		return (*mm_results).s1
	}
	if mmCwd.funcCwd != nil {
		return mmCwd.funcCwd()
	}
	mmCwd.t.Fatalf("Unexpected call to ExecutionContextMock.Cwd.")
	return
}

// CwdAfterCounter returns a count of finished ExecutionContextMock.Cwd invocations
func (mmCwd *ExecutionContextMock) CwdAfterCounter() uint64 {
	return mm_atomic.LoadUint64(&mmCwd.afterCwdCounter)
}

// CwdBeforeCounter returns a count of ExecutionContextMock.Cwd invocations
func (mmCwd *ExecutionContextMock) CwdBeforeCounter() uint64 {
	return mm_atomic.LoadUint64(&mmCwd.beforeCwdCounter)
}

// MinimockCwdDone returns true if the count of the Cwd invocations corresponds
// the number of defined expectations
func (m *ExecutionContextMock) MinimockCwdDone() bool {
	for _, e := range m.CwdMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			return false
		}
	}
	...
	// if default expectation was set then invocations count should be greater than zero
	if m.CwdMock.defaultExpectation != nil && mm_atomic.LoadUint64(&m.afterCwdCounter) < 1 {
		return false
	}
	// if func was set then invocations count should be greater than zero
	if m.funcCwd != nil && mm_atomic.LoadUint64(&m.afterCwdCounter) < 1 {
		return false
	}
	return true
}

// MinimockCwdInspect logs each unmet expectation
func (m *ExecutionContextMock) MinimockCwdInspect() {
	for _, e := range m.CwdMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Error("Expected call to ExecutionContextMock.Cwd")
		}
	}
