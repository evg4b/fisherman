...
1. Create `core/src/rules/<rule_name>.rs` with a struct that derives `serde::Serialize/Deserialize` and (if it has a
   `when` field) `ConditionalRuleDerive`.
...
- Doc-tests are **disabled** for the `core` crate (`[lib] doctest = false`) because the crate name `core` shadows the
  stdlib `core` crate in rustdoc's `--extern` flags
