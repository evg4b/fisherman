...
impl CompiledRule for CopyFiles {
    fn check(&self, _: &dyn Context) -> Result<RuleResult> {
        let compiled_glob = self.glob.to_string()?;
        let compiled_src = self.src.as_ref().map(|s| s.to_string()).transpose()?;
        let compiled_pattern = match compiled_src.clone() {
            Some(src) => Path::join(src.as_ref(), compiled_glob),
            None => compiled_glob.parse()?,
        };
        let compiled_destination = self.destination.to_string()?;
        ...
        Ok(RuleResult::Success {
            ...
            output: format!("Copied {} files", copied_files),
        })
    }
}
